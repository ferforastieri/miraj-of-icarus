...
const realmSystems = [
  {
    title: "Clãs",
    subtitle: "Uma bandeira compartilhada",
    description: "Jogadores poderão reunir aliados, organizar funções e construir uma identidade reconhecida em todo o reino.",
  },
  {
    title: "Guerras",
    subtitle: "Conflitos com propósito",
    description: "Disputas planejadas colocarão clãs frente a frente por objetivos claros, temporadas e reconhecimento coletivo.",
  },
  {
    title: "Territórios",
    subtitle: "O mapa guarda memória",
    description: "Regiões conquistáveis deverão registrar domínio, abrir rotas estratégicas e transformar a relação entre os clãs.",
  },
] as const;
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#041d19]">
      <section className="relative isolate grid min-h-[76svh] place-items-center overflow-hidden px-6 pb-20 pt-56 text-center max-[700px]:min-h-[700px] max-[700px]:px-4 max-[700px]:pt-32">
        ...
        <div className="w-[min(920px,100%)]">
          <p className="font-miraj-of-icarus text-xs uppercase tracking-[.25em] text-[#b5e8c8]">Além do grande portal</p>
          <h1 className="mt-5 font-miraj-of-icarus text-[clamp(3.2rem,7vw,7rem)] font-semibold leading-[.82] text-[#f5f0dc] [text-shadow:0_4px_16px_#031b16]">Cada reino guarda uma disputa.</h1>
          <p className="mx-auto mt-8 max-w-3xl text-[clamp(1rem,1.6vw,1.25rem)] leading-8 text-[#e2ebe2]">Os reinos são mundos de jogo independentes. Neles, personagens crescem, clãs erguem bandeiras e futuras guerras poderão redesenhar o controle dos territórios.</p>
          <Link className={`${buttonStyles("primary", true)} mt-9`} href="#reinos-disponiveis">Ver reinos disponíveis</Link>
        </div>
      </section>
        ...
        <section className="bg-[linear-gradient(180deg,#edf0df,#f8f1de)] px-6 py-28 text-[#173b32] max-[700px]:px-4 max-[700px]:py-20" id="reinos-disponiveis">
          <div className="mx-auto w-[min(1120px,100%)]">
            <header className="grid grid-cols-[.6fr_1.4fr] gap-12 border-b border-[#9c824d] pb-12 max-[760px]:grid-cols-1 max-[760px]:gap-4">
              <p className="font-miraj-of-icarus text-xs uppercase tracking-[.2em] text-[#927038]">Passagens disponíveis</p>
              <h2 className="font-miraj-of-icarus text-[clamp(2.7rem,5vw,5rem)] leading-[.88]">Escolha onde sua história será escrita.</h2>
            </header>
            <div className="mt-10 grid gap-3">
              {servers.data?.map(server => (
                <article className="grid min-h-32 grid-cols-[1fr_auto] items-center border-y border-[#a58a52] bg-[#edf0df] px-8 py-6 max-[560px]:grid-cols-1 max-[560px]:gap-4 max-[560px]:px-5" key={server.id}>
                  ...
                  <span className={`font-miraj-of-icarus text-sm uppercase tracking-[.14em] ${server.available ? "text-[#19734f]" : "text-[#8b6454]"}`}>{server.available ? "Disponível" : "Manutenção"}</span>
                </article>
              ))}
              {servers.isLoading && <p className="border-y border-[#a58a52] py-12 text-center text-[#5e7168]">Consultando as passagens...</p>}
              {!servers.isLoading && !servers.data?.length && <p className="border-y border-[#a58a52] py-12 text-center text-[#5e7168]">Nenhum reino pôde ser consultado agora. Tente novamente em alguns instantes.</p>}
            </div>
          </div>
        </section>
        ...
        <section className="relative isolate overflow-hidden bg-[#052721] px-6 py-28 text-[#eef1e7] max-[700px]:px-4 max-[700px]:py-20">
          ...
          <div className="mx-auto w-[min(1180px,100%)]">
            <header className="mx-auto max-w-4xl text-center">
              <p className="font-miraj-of-icarus text-xs uppercase tracking-[.22em] text-[#92dfaf]">Sistemas em planejamento</p>
              <h2 className="mt-4 font-miraj-of-icarus text-[clamp(2.8rem,5.5vw,5.5rem)] leading-[.88] text-[#f4efdc]">Uma fronteira definida por seus jogadores.</h2>
              <p className="mx-auto mt-7 max-w-2xl leading-7 text-[#bfcfc4]">Clãs, guerras e territórios ainda estão em desenvolvimento. Esta é a direção prevista para transformar cada reino em uma história coletiva.</p>
            </header>
            ...
            <div className="mt-14 text-center">
              <Link className={buttonStyles("primary", true)} href={routes.community}>Acompanhar a comunidade</Link>
            </div>
          </div>
        </section>
      </main>
