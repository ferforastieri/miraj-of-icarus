    ...
    <div className="min-h-screen bg-[#041d19]">
      <section className="relative isolate grid min-h-[82svh] place-items-center overflow-hidden px-6 pb-20 pt-56 text-center max-[700px]:px-4 max-[700px]:pt-32">
        ...
        <div className="w-[min(850px,100%)]">
          <p className="font-miraj-of-icarus text-xs uppercase tracking-[.24em] text-[#a9e9c4]">A próxima jornada</p>
          <h1 className="mt-4 font-miraj-of-icarus text-[clamp(3.1rem,7vw,6.8rem)] leading-[.84] text-[#f4f1df] [text-shadow:0_3px_12px_#031b16]">Prepare sua passagem.</h1>
          <p className="mx-auto mt-7 max-w-2xl text-lg leading-8 text-[#d9e5da]">O launcher instala o cliente, verifica seus arquivos e mantém o acesso aos reinos pronto para começar.</p>
          ...
          {release && <p className="mt-5 text-xs uppercase tracking-[.12em] text-[#c9d8ca]">Versão {release.version.slice(0, 8)} · {formatBytes(release.totalSize)} · {new Intl.DateTimeFormat("pt-BR").format(new Date(release.publishedAt))}</p>}
          {releaseQuery.isError && <p className="mx-auto mt-6 max-w-xl text-sm text-[#d8cfae]">Não foi possível consultar a versão agora. Tente novamente em alguns minutos.</p>}
        </div>
      </section>
      <SiteFooter />
    </div>
