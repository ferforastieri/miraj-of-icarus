  ...
  return (
    <div className="min-h-screen bg-abyss"><SiteHeader />
      <main className="mx-auto w-[min(1180px,calc(100%-40px))] pb-16 pt-52 max-[700px]:pt-32 max-[560px]:w-[calc(100%-18px)]">
        ...
        {selectedAccount !== null && <section className="jade-card mb-16"><div className="mb-5 flex items-center justify-between"><div><Kicker>{t("account")} #{selectedAccount}</Kicker><h2 className="text-4xl">{t("characters")}</h2></div><Button onClick={() => setSelectedAccount(null)}>{t("close")}</Button></div><div className="grid gap-2">{characters.data?.map(character => { const knownClass = gameClasses.find(item => item.id === character.archetype); return <article className="flex items-center justify-between gap-4 border-y border-jade/20 p-4 max-[560px]:flex-col max-[560px]:items-start" key={character.id}><div><strong className="text-xl">{character.name}</strong><p className="text-sm text-mist">{knownClass ? classesT(`items.${knownClass.id}.name`) : character.archetype} · {t("level", { level: character.level })}{character.deletionScheduledAt ? ` · ${t("deletionScheduled")}` : ""}</p></div>{character.deletionScheduledAt ? <Button onClick={() => restoreCharacter.mutate({ accountId: selectedAccount, characterId: character.id })}>{t("cancelDeletion")}</Button> : <Button variant="danger" onClick={() => { if (window.confirm(t("scheduleDeletionQuestion", { name: character.name }))) deleteCharacter.mutate({ accountId: selectedAccount, characterId: character.id }); }}>{t("scheduleDeletion")}</Button>}</article>; })}</div></section>}
