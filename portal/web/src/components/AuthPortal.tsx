    ...
    <main className="relative grid min-h-[calc(100vh-142px)] place-items-center bg-[linear-gradient(90deg,rgba(7,30,50,.82),rgba(7,30,50,.3),rgba(7,30,50,.7)),url('/media/portal-hero-v3.png')] bg-cover bg-center px-6 py-20 max-[620px]:px-4">
      ...
      <section className="jade-card relative w-[min(620px,100%)] px-8 py-7 drop-shadow-[0_30px_60px_rgba(3,27,22,.48)] max-[620px]:px-2 max-[620px]:py-5" aria-labelledby="auth-title">
        ...
        <div className="my-6 grid grid-cols-2 gap-2" role="tablist" aria-label="Acesso à conta">
          <Link className={`${tabClass} ${!registering ? "text-white drop-shadow-[0_0_8px_rgba(40,185,111,.65)]" : ""}`} style={{ backgroundImage: `url('/media/game-ui/jade/button-${!registering ? "focused" : "default"}.png')` }} role="tab" aria-selected={!registering} href={routes.panel}>Entrar</Link>
          <Link className={`${tabClass} ${registering ? "text-white drop-shadow-[0_0_8px_rgba(40,185,111,.65)]" : ""}`} style={{ backgroundImage: `url('/media/game-ui/jade/button-${registering ? "focused" : "default"}.png')` }} role="tab" aria-selected={registering} href={`${routes.panel}?modo=cadastro`}>Criar conta</Link>
        </div>
